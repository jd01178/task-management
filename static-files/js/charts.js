	/*chart-employment*/
	 ...
	 // let chartDonutData = sendGetRequest('')
	// Function to form pairs from input string
// Assume you have fetched the data from the backend and stored it in a variable called 'data'
// The 'data' variable should contain the JSON data provided by the server

// Create a function to form pairs from the input string
// Function to create donut chart
function createDonutChart(chartId, data) {
  var chart = c3.generate({
    bindto: chartId,
    data: {
      columns: data,
      ...
      names: {
        data1: 'Tasks Not Started',
        data2: 'Tasks In Progress',
        data3: 'Tasks To be Reviewed',
        data4: 'Completed Tasks',
      }
    },
    axis: {},
    legend: {
...
// Function to parse the data received from the backend
function parseChartData(rawData) {
  const parsedData = {};
  for (const key in rawData) {
    parsedData[key] = rawData[key].map(item => [item[0], item[1]]);
  }
  return parsedData;
}
...
fetch('/api/v1/tasks-evaluation/')
  .then(response => response.json())
  .then(data => {
	// Call the function to create donut charts with the received data
	const parsedData = parseChartData(data);
	createDonutChart('#chart-donut', parsedData.daily);
	createDonutChart('#chart-donut2', parsedData.weekly);
	createDonutChart('#chart-donut3', parsedData.monthly);
	createDonutChart('#personal-chart-donut', parsedData.personal_daily);
	createDonutChart('#personal-chart-donut2', parsedData.personal_weekly);
	createDonutChart('#personal-chart-donut3', parsedData.personal_monthly);
  })
  .catch(error => {
    console.error(error);
  });
